...
Double_t maxBinM = maxBinP;
Double_t maxBinO = maxBinP;
...
//OUTRAS PARTICULAS
TH1D* histo_Outras = new TH1D("OutrasParticulas", "Outras Particulas", nBins, minBin, maxBinO);
newTree->Draw("soma>>OutrasParticulas", "particlePDGNew != 13 || particlePDGNew !=-13 || particlePDGNew !=211 || particlePDGNew !=-211");//Seleção da energia total das restantes particulas, utilizando as condições particlePDGNew != -13 ou particlePDGNew != 13 ou particlePDGNew != 211 ou particlePDGNew != -211, que são todas as particulas que não são muões ou piões
...
gPad->SetLogy();
cs->BuildLegend();
//cs->SaveAs("/home/marta/Desktop/TEC2_Trabalho/Graficos/alinea3/Histograma_ETotal_Particula"+root_file(15,16)+".png");
